// src/components/Navbar.jsx import React, { useState } from 'react'; import { Link } from 'react-router-dom'; import { Menu as MenuIcon, X as XIcon } from 'react-feather'; import ThemeToggle from './ThemeToggle'; import LanguageSwitcher from './LanguageSwitcher'; import Logo from '../assets/dealcross-logo.png';
...
{/* Desktop Links */}
    ...
    <div className="hidden md:flex space-x-2">
      <Link to="/login" className="px-3 py-1.5 text-sm bg-blue-600 hover:bg-blue-700 text-white rounded-md">Login</Link>
      <Link to="/signup" className="px-3 py-1.5 text-sm bg-gray-800 hover:bg-gray-700 text-white rounded-md">Sign Up</Link>
    </div>
    ...
    <>
      ...
      <div className="fixed top-0 right-0 w-3/4 h-full bg-white dark:bg-gray-900 p-6 space-y-4 z-50">
        ...
        <div className="flex space-x-2">
          <Link to="/login" onClick={() => setOpen(false)} className="px-3 py-1.5 text-sm bg-blue-600 hover:bg-blue-700 text-white rounded-md">Login</Link>
          <Link to="/signup" onClick={() => setOpen(false)} className="px-3 py-1.5 text-sm bg-gray-800 hover:bg-gray-700 text-white rounded-md">Sign Up</Link>
        </div>
