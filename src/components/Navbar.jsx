// src/components/Navbar.jsx import React, { useState } from 'react'; import { Link } from 'react-router-dom'; import { Menu as MenuIcon, X as XIcon } from 'react-feather'; import Logo from '../assets/dealcross-logo.png'; import LanguageSwitcher from './LanguageSwitcher'; import ThemeToggle from './ThemeToggle';
...
{/* Desktop Links */}
    ...
    <div className="hidden md:flex space-x-2 items-center">
      <ThemeToggle />
      <LanguageSwitcher />
      <Link to="/login" className="text-sm px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-full">Login</Link>
      <Link to="/signup" className="text-sm px-3 py-1.5 bg-gray-800 hover:bg-gray-700 text-white rounded-full">Sign Up</Link>
    </div>
    ...
    <>
      ...
      <div className="fixed top-0 right-0 w-3/4 h-full bg-white dark:bg-gray-900 p-6 space-y-4 z-50">
        ...
        <div className="flex space-x-4 pt-4">
          <Link to="/login" onClick={() => setOpen(false)} className="w-10 h-10 flex items-center justify-center bg-blue-600 hover:bg-blue-700 text-white rounded-full text-sm">Login</Link>
          <Link to="/signup" onClick={() => setOpen(false)} className="w-10 h-10 flex items-center justify-center bg-gray-800 hover:bg-gray-700 text-white rounded-full text-sm">Sign</Link>
        </div>
      </div>
    </>
  )}
