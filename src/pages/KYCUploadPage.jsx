...
import React, { useState } from 'react'; import axios from 'axios'; import { Helmet } from 'react-helmet';

const KYCUploadPage = () => { const [form, setForm] = useState({ user_id: '', document_type: '', document_file: null, });

const [status, setStatus] = useState({ type: '', message: '' }); const [loading, setLoading] = useState(false);

const handleChange = (e) => { const { name, value, files } = e.target; if (name === 'document_file') { setForm({ ...form, document_file: files[0] }); } else { setForm({ ...form, [name]: value }); } };

const resetForm = () => { setForm({ user_id: '', document_type: '', document_file: null }); };
...
return ( <> <Helmet> <title>KYC Upload - Dealcross</title> <meta name="description" content="Submit your KYC for verification on Dealcross" /> </Helmet>
