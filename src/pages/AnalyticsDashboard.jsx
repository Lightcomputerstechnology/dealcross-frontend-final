...
const AnalyticsDashboard = () => { const [stats, setStats] = useState(null); const [status, setStatus] = useState('Loading analytics...'); const [lastUpdated, setLastUpdated] = useState(null);

const fetchStats = async () => { const token = localStorage.getItem('token'); if (!token) { setStatus('Admin login required.'); return; }
...
useEffect(() => { fetchStats(); const interval = setInterval(fetchStats, 20000); // refresh every 20s return () => clearInterval(interval); }, []);

return ( <div className="min-h-screen bg-[#0f172a] text-white px-6 py-10"> <Helmet> <title>Admin Analytics - Dealcross</title> <meta name="description" content="View real-time admin analytics and statistics on Dealcross." /> <meta name="keywords" content="dealcross, analytics, admin, metrics, stats, dashboard" /> <meta name="author" content="Dealcross Team" /> </Helmet>

<h2 className="text-2xl font-bold mb-4">Admin Analytics</h2>
  {lastUpdated && (
    <p className="text-sm text-gray-400 mb-2">Last updated at: {lastUpdated}</p>
  )}
